program day13p2
  implicit none
  integer :: info
  integer(kind=8) :: intcode(0:32766),prog(0:0,0:32766),sii(2),pos(0:0)
  integer(kind=8) :: rel(0:0),exitopcode,xres,yres,tres
  integer(kind=8) :: score
  complex         :: ball,paddle

  write(*,'(a)')"Advent of Code 2019 day 13, part 2"
  write(*,'(a)')""
...
! Initialise the intcode computer, program address 0 is 2 for free play
! Input is from the joystick ... follow the change in ball x to determine
! if this should be 0 (no change), -1 (go left), or 1 (go right)
  prog(0,0:)=intcode
  prog(0,0)=2
  pos(0)=0
  rel(0)=0
...
! Initialise the ball and paddle (-1,-1 means we've not seen them before)
  ball=complex(-1,-1)
  paddle=complex(-1,-1)

  do
    call runprogram(0_8,32766_8,prog(0,0:),pos(0),rel(0),sii(1),1_8,xres,exitopcode)
!   First output is the x value
    if (exitopcode == 99) exit

    call runprogram(0_8,32766_8,prog(0,0:),pos(0),rel(0),sii(1),1_8,yres,exitopcode)
!   Second output is the y value
    if (exitopcode == 99) exit

    call runprogram(0_8,32766_8,prog(0,0:),pos(0),rel(0),sii(1),1_8,tres,exitopcode)

!   Third output is the tile id, unless xres was -1 and yres was 0, in which
!   case it's the score
    if (exitopcode == 99) exit

    if ((xres == -1).and.(yres == 0)) then
!     write(*,'(a,i6)') "score ",tres
      score=tres
    end if

    if (tres == 3) then 
!     write(*,'(a,2i3)') "paddle",xres,yres
      paddle=complex(xres,yres)
    end if

    if (tres == 4) then 
!     write(*,'(a,2i3)') "ball  ",xres,yres
      ball=complex(xres,yres)
!     Don't move the paddle if we've not yet seen it on the screen
      if (int(real(paddle)) /= -1) then
        sii(1)=0
        if (int(real(ball))>int(real(paddle))) sii(1)= 1
        if (int(real(ball))<int(real(paddle))) sii(1)= -1
      end if
    end if
  ...
  write(*,'(a,i8)')"Score after the last block is broken:",score
