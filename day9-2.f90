program day9p2
  ...
  integer(kind=8) :: exitopcode

  write(*,'(a)')"Advent of Code 2019 day 9, part 2"
  write(*,'(a)')""
! Initialise intcode array to 0
  intcode = 0
  open(10,file="day9in.txt")
  do
  ...
  prog(0,0:)=intcode
  pos(0)=0
! Set the input value to 2 (distress signal mode)
  sii(1)=2
  do
    call runprogram(0_8,32766_8,prog(0,0:),pos(0),sii(1),1_8,result,exitopcode)
!   write(*,*)"Exit opcode is ",exitopcode
    if (exitopcode == 99) exit
    write(*,'(a,i12)') "Result is ",result
  end do

  write(*,'(a)')""
  write(*,'(a,i12)')"The distress signal co-ordinates are ",result
...
subroutine runprogram(m,n,tape,pos,sii,ninputs,result,exitopcode)
  ...
  integer(kind=8),intent(inout) :: tape(m:n),pos
  integer(kind=8),intent(out)   :: result,exitopcode
  integer(kind=8),save          :: relativebase=0
...
! write(*,*)"relative base now ",relativebase
...
!     addition
      case (1)
        call getoperands(m,n,tape,pos,mode,operands,2_8,relativebase)
        if (mode(3) == 0) tape(tape(pos+3))=operands(1)+operands(2)
        if (mode(3) == 2) tape(relativebase+tape(pos+3))=operands(1)+operands(2)
        exitopcode=1
        pos=pos+4
...
!     single integer input
      case (3)
        if (siin > ninputs) then
          ...
          if (mode(1) == 2) tape(relativebase+tape(pos+1))=sii(siin)
        end if
...
!     single integer output
      case (4)
        call getoperands(m,n,tape,pos,mode,operands,1_8,relativebase)
        result=operands(1)
        pos=pos+2
...
!     less than
      case (7)
        ...
        if (operands(1) < operands(2)) then
          ...
          if (mode(3) == 2) tape(relativebase+tape(pos+3))=1
        else
          if (mode(3) == 0) tape(tape(pos+3))=0
          if (mode(3) == 2) tape(relativebase+tape(pos+3))=0
        end if
        exitopcode=7
        pos=pos+4
...
!     relative base adjustment
      case (9)
        call getoperands(m,n,tape,pos,mode,operands,1_8,relativebase)
        relativebase=relativebase+operands(1)
        exitopcode=9
        pos=pos+2
