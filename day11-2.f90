program day11p2
  ...
  complex                 :: nextpanel
  character (len=1600)    :: grid
  integer                 :: npanels,direction,l1,x,y
  integer(kind=8),dimension(5000) :: colour
  logical                 :: seenpanel

  write(*,'(a)')"Advent of Code 2019 day 11, part 2"
  write(*,'(a)')""
...
! All panels are black to start with
  colour=0
! Our first (next) panel is at the origin, 10,10 (means we don't have to
! deal with negative co-ordinates)
  npanels=1
  l1=npanels
  points(npanels)=complex(10,10)
...
! Initialise the intcode computer
  ...
  pos(0)=0
  rel(0)=0
! Set the first input value to 1 (robot starts over a white panel, value 0)
  sii(1)=1
  do
...
! Initialise the printout grid
  grid=' '

! Paint the white tiles
  do l1=1,npanels
    if (colour(l1) == 1) then
      x=int(real(points(l1)))
      y=int(aimag(points(l1)))
      grid(y*80+x+1:y*80+x+1)='X'
    end if
  end do

! Print the grid
  do y=1600,1,-80
    write(*,'(a80)')grid(y-79:y)
  end do
